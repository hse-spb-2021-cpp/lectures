...
### 12. Объявление объектов внутри одного файла
...
* Арифметика указателей, one-past-the-last, array-to-pointer decay, арифметика с `void*` (`08-211020\00-extra`)
...
### 24. Друзья и перегрузка операторов
* Друзья-классы.
* Друзья-функции.
...
* Использование функторов.
  ...
  * Сохранение в поле класса/переменную при помощи `std::function`
...
### 32. Программы из нескольких файлов
* Мотивация.
...
### 34. Static initialization order fiasco
...
* Cross-cast (он же side-cast), delegate-to-sister
* Как теперь работают `dynamic_cast`/`static_cast`/неявное преобразование/`override`/hiding/приватное и защищённое наследование
  * В том числе при наличии одинаковых виртуальных функций в независимых базах
...
### 46. Разделяемые библиотеки
`11-211124/02-shared-library`

* Создание и использование разделяемой библиотеки через Boost.DLL для системы плагинов на основе полиморфных классов
* Пример компилирующегося и работающего кода
* Независимые по умолчанию static storage у приложения и разделяемой библиотеки
* Необходимость совпадения стандартных библиотек при компиляции
  * Не было: необходимость динамической линковки стандартной библиотеки C++
* Жизненный цикл разделяемой библиотеки: загрузка, выгрузка; что происходит с объектами
